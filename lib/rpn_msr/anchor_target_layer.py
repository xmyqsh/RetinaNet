...
def anchor_target_layer(rpn_cls_score_P3, \
                        rpn_cls_score_P4, \
                        rpn_cls_score_P5, \
                        rpn_cls_score_P6, \
                        rpn_cls_score_P7, \
    ...
    print num_anchor_scale
    print "type(anchor_sizes[0]):"
    print type(anchor_sizes[0])
    print anchor_sizes[0]
    ...
    for idx, rpn_cls_score in enumerate([rpn_cls_score_P3, rpn_cls_score_P4, rpn_cls_score_P5, rpn_cls_score_P6, rpn_cls_score_P7]):
...
#    print "fg: {:d}, bg: {:d}, disabled: {:d}".format(np.sum(labels == 1), np.sum(labels == 0), np.sum(labels == -1))
    ...
    bbox_inside_weights[labels > 0, :] = np.array(cfg.TRAIN.RPN_BBOX_INSIDE_WEIGHTS)
